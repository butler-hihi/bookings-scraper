...
class Scheduler:
    ...
    def run_once(self) -> dict:
        ...
        results = {}
        all_changes = {"newly_available": [], "newly_unavailable": [], "trail_names": []}
            ...
            if trail_result:
                ...
                if status == "success":
                    trail_changes = self._extract_changes(
                        trail_name=trail_name, results=trail_result
                    )
                    ...
                    all_changes["newly_unavailable"].extend(
                        trail_changes.get("newly_unavailable", [])
                    )
    ...
    def run_loop(self, interval_seconds: int = 60) -> None:
        ...
        while self.running:
            try:
                ...
                logger.info(
                    f"Check completed: {result['success_count']}/{len(self.trails)} trails successful"
                )
