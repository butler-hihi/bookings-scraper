...
def save_availability(
    ...
    try:
        ...
        existing = session.query(AvailabilityRecord).filter(
            AvailabilityRecord.trail_name == trail_name,
            AvailabilityRecord.date == date
        ).first()
...
def get_availability(
    ...
    try:
        record = session.query(AvailabilityRecord).filter(
            AvailabilityRecord.trail_name == trail_name,
            AvailabilityRecord.date == date
        ).first()
...
def get_changes(
    trail_name: str,
    current_availability: Dict[str, bool],
    db_path: Path = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        records = session.query(AvailabilityRecord).filter(
            AvailabilityRecord.trail_name == trail_name
        ).all()
        
        # Convert to dict for comparison
        stored_availability = {
            record.date: record.available
